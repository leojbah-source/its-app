...
router.post('/participant/:id/payment', authenticate, async (req, res, next) => {
  try {
    ...
    // No email here — the single confirmation email is sent only when the parent
    // completes the registration (POST /participant/:id/confirm).
    res.status(201).json(rows[0]);
  } catch (err) { next(err); }
});
  ...
  try {
    const { rows: pRows } = await pool.query(
      `SELECT p.id, p.year_id, p.full_name, p.cpr_number, p.dob, p.gender,
              ...
              ag.label AS age_group_label, s.name AS school_name
       FROM participants p
       LEFT JOIN age_groups ag ON ag.id = p.age_group_id
       LEFT JOIN schools s ON s.id = p.school_id
    ...
    if (parent.email) {
      const result = await sendEmail({
        ...
        html: registrationConfirmationHtml({
          ...
          participant: p,
          items, payments: pays,
          summary: { fees_total: feesTotal, paid_confirmed: paidConfirmed, balance_due: feesTotal - paidConfirmed },
        }),
      });
