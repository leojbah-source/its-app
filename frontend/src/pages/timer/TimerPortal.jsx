...
// Timer portal (staff Timer role). Chest numbers ONLY. Start/stop a stopwatch
// per chest; yellow at (allotted − yellow), red at allotted, grace after. On
// stop the time is written against the chest. Only a Chairman (email+password)
// can correct a missed time.
...
export default function TimerPortal() {
  ...
  const [edit, setEdit] = useState(null);        // { registration_id, chest_number }
  const timer = useRef(null);

  useEffect(() => {
    timerApi.myEvents(token).then((evs) => { setEvents(evs); if (evs.length === 1) setEventId(evs[0].event_id); }).catch((e) => setFlash(e.message));
  }, [token]);

  const load = useCallback(async () => {
    if (!eventId) { setData(null); return; }
    try { setData(await timerApi.participants(token, eventId)); } catch (e) { setFlash(e.message); }
  }, [token, eventId]);
  useEffect(() => { load(); }, [load]);

  // tick every 250ms for the live clock; poll data every 3s
  useEffect(() => {
  ...
  function lightState(sec) {
    const allotted = Number(th.allotted_time_seconds) || 0;
    const yellow = Number(th.yellow_alert_seconds) || 0;
    const grace = Number(th.grace_period_seconds) || 0;
  ...
  async function start(p) { try { await timerApi.start(token, eventId, { registration_id: p.registration_id, chest_number: p.chest_number }); setNow(Date.now()); load(); } catch (e) { setFlash(e.message); } }
  async function stop(p) { try { await timerApi.stop(token, eventId, p.registration_id); load(); } catch (e) { setFlash(e.message); } }
      ...
      <main className="mx-auto max-w-2xl p-4">
        ...
        {!eventId ? (
          ...
          <div>
            ...
            {/* Big live timer for the running chest */}
            {running && (() => { const ls = lightState(elapsed); return (
              <div className="mb-4 rounded-2xl bg-navy-800 p-6 text-center text-white">
                ...
                <div className="my-2 font-mono text-6xl font-black tabular-nums" style={{ fontSize: 'clamp(3rem,18vw,7rem)' }}>{clock(elapsed)}</div>
                <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm">
                  <span className={`h-3 w-3 rounded-full ${ls.c}`} /> {ls.label} · allowed {clock(th.allotted_time_seconds)}
                </div>
            ...
            <div className="rounded-xl bg-white p-2 shadow-sm">
              <ul className="divide-y divide-slate-100">
                {data.participants.map((p) => {
                  const isRunning = p.start_time && !p.end_time;
                  const done = !!p.end_time;
                  return (
                    <li key={p.registration_id} className="flex items-center justify-between px-3 py-2.5">
                      <span className="flex items-center gap-3">
                        ...
                        {done ? <span className={`text-sm font-medium ${p.flag_for_dq ? 'text-red-600' : 'text-green-600'}`}>{clock(p.time_taken_seconds)}{p.flag_for_dq ? ' · over' : ''}</span>
                          : isRunning ? <span className="text-sm text-amber-600">running…</span>
                ...
                {data.participants.length === 0 && <li className="px-3 py-6 text-center text-sm text-slate-400">No chest numbers assigned yet.</li>}
              </ul>
            </div>
        ...
        {edit && <OverrideModal edit={edit} token={token} eventId={eventId} onClose={() => setEdit(null)} onDone={() => { setEdit(null); load(); }} setFlash={setFlash} />}
      </main>
    </div>
  );
}

function OverrideModal({ edit, token, eventId, onClose, onDone, setFlash }) {
